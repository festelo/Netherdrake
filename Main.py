...
class AccountsTable(QTableWidget):
    ...
    def getRowInformation(self, id) -> tuple:
        id = id + 1
        current_account_id = self.item(id, 0).text()
        current_account_login = self.item(id, 1).text()
        current_account_password = self.item(id, 2).text()
        current_account_status = self.item(id, 3).text()
    ...
    def TryToDoCommentaries(self, event):
        ...
        if event:
            ...
            users[id-1].isWorking = True
            threading.Thread(target=self.doCommentaries, args=(id,)).start()
        else:
            self.setItem(id, 3, QTableWidgetItem('Не работает'))
            users[id-1].isWorking = False

    def doCommentaries(self, id):
        try:
            while users[id-1].isWorking:
                self.workWithVK(users[id-1])
        except:
            self.setItem(id, 3, QTableWidgetItem('Не работает'))
            users[id-1].isWorking = False
